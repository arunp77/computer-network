...
3. **What happens when you type `google.com` into your browser and press Enter?**
   *Answer:* (See Module 17 for full details). Briefly: DNS resolution $\rightarrow$ TCP Handshake $\rightarrow$ TLS Handshake $\rightarrow$ HTTP GET request $\rightarrow$ Server responds with HTML $\rightarrow$ Browser renders.
...
9. **Explain the TCP 3-Way Handshake.**
   *Answer:* It establishes a TCP connection. 1) Client sends SYN. 2) Server responds with SYN-ACK. 3) Client responds with ACK.
